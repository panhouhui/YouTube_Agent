...
def get_proxy_from_env_or_windows() -> Optional[str]:
...
class AioHttpClient(HttpClient):
    ...
    def __init__(self, logger: logging.Logger, cookies_file: Optional[Path], headers: Optional[Dict[str, Any]]):
        ...
        session = aiohttp.ClientSession(cookie_jar=cookies, headers=headers, trust_env=True)
        self.session = session
        self.proxy = get_proxy_from_env_or_windows()
        if self.proxy:
    ...
    async def request_once(self, url: str,
        ...
        try:
            async with self.session.request(method, url, headers=request_headers, params=params, data=data,
                                            json=data_json, server_hostname=server_hostname,
                                            proxy=self.proxy) as client_response:
...
class CurlCffiHttpClient(HttpClient):
    ...
    def __init__(self, logger: logging.Logger, cookies_file: Optional[Path], headers: Optional[Dict[str, Any]]):
        ...
        self.session: curl_cffi.requests.AsyncSession = curl_cffi.requests.AsyncSession(
